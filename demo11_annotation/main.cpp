...
x3::rule<quoted_string_class, std::string> const quoted_string =
    "quoted_string";
...
auto const employee_def = '{' >> int_ >> ',' >> person >> ',' >> double_ >> '}';
...
std::vector<client::ast::employee> parse(std::string const& input,
  ...
  auto const parser =
      ...
      with<position_cache_tag>(std::ref(positions))[employees];
